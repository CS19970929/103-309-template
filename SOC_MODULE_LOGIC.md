...
2. 满电确认：端点电压可信并持续约 60s 后，把内部 SOC 设为 100%，并触发可信满电锚点；不再强依赖充电器进入恒压/小电流尾段。
...
| 条件 | 当前逻辑 |
| --- | --- |
| 模式 | 不能有有效放电电流；`CHG` 或充电截止后的 `RELAX` 都允许 |
| 最高单体 | `VCellMax >= V100` |
| 最低单体 | `VCellMin >= max(化学体系最低门槛, V100 - 80mV)` |
| 单体压差 | `abs(VCellMax - VCellMin) <= 120mV`；配置为 `0` 时不检查 |
| 误触发保护 | 需满足以下任一条件：内部 SOC 已到 `95%`、`CapNow` 已到满容量 `95%`、或 `Ichg != 0` 且 `Ichg <= C/20` |
| 持续时间 | `SOC_FULL_CONFIRM_SECONDS = 60s` |

当前 `TERNARYLI` 的化学体系最低门槛为 `4000mV`，`LIFEPO/LIFEPO2` 为 `3300mV`。在 27Ah 三元锂默认 `V100 = 4180mV` 下，实际最低单体确认阈值为 `4100mV`；默认 taper 电流为 `270 / 20 = 13`，单位 `A * 10`，约 1.3A。新的满电确认允许三种路径：真实小电流尾段、容量/SOC 已接近满、或充电截止后无电流但端点电压仍可信，因此不要求这些条件同时出现。
