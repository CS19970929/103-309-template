...
8. 不把“满足 30min 静置”作为唯一校准入口；短暂停车且电压稳定时也允许慢速小步收敛。
...
| 短静置稳定校准 | `5min` 稳定后，每 `10min` 允许 `1%` | 静置不足 30min 但电压可信时慢速收敛 |
...
这张表的定位是“可信 OCV 参考”，不是所有电芯、所有温度、所有负载下的真实 SOC。运行态只在启动无快照、静置/RTC、小步短静置和受限弱约束场景使用它，避免把电芯批次、老化、温度或内阻差异直接放大成用户可见跳变。
...
| `stable_rest/short_rest` ticks | 短静置稳定窗口和短静置小步校准节拍 |
| `mid_ticks` | 中低压弱约束每 `1%` 下修计数 |
| `rest_ref_vmin/rest_ref_vmax` | 短静置稳定性参考电压 |
...
短静置补偿用于覆盖“用户停车时间不足 30min，但电压已经稳定”的场景：

- 只在 `RELAX` 下运行。
- 电压合法、单体压差 `<=200mV`。
- 没有大电流 sag/rebound holdoff。
- `VCellMin/VCellMax` 相对短静置参考值波动 `<=30mV`。
- 连续稳定 `5min` 后，每累计 `10min` 最多按 OCV 表修正 `1%`。
- 如果中途电压跳动、重新骑行、进入低压表或触发 holdoff，短静置可信度清零。
