...
- `NORMAL_MODE` / `DEEP_MODE` 当前仍属于“写 BKP 标志 + AFE Sleep + MCU 复位 + 启动早期 Stop”的旧路径，短期应保留兼容，长期建议并入统一低功耗执行器。
...
`LowPower_Request()` 只设置 `g_sleepModeSelect` 和 `Sleep_Mode` 强制位，真正进入休眠在后续 `rtc_sleep()` 分支内完成，见 `rtc_sleep.c:324` 到 `rtc_sleep.c:355`、`rtc_sleep.c:1052` 到 `rtc_sleep.c:1102`。
...
| DI 长按 | LED 按键长按达到 3 秒 | `DEEP_MODE`，并立即 `SleepDeal_Continue()` | `LedBar.c:739` 到 `LedBar.c:755` |
...
4. 模式提交收敛：让 `LowPower_Request()` 成为唯一模式入口，拆掉 `SleepDeal_Continue()` 里的二次模式选择。
