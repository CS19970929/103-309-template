...
/* PG1 */
#define BEEP_GPIO_CLK_ENABLE()      RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);
...
void BEEP_InitHard(void)
{
    ...
    gpio_init.GPIO_Mode = GPIO_Mode_Out_PP; /* 设置开漏输出 */
    gpio_init.GPIO_Speed = GPIO_Speed_2MHz; /* GPIO速度等级 */

    gpio_init.GPIO_Pin = GPIO_PIN_BEEP;
    GPIO_Init(GPIO_PORT_BEEP, &gpio_init);
#endif
...
{
    BEEP_Start(5, 1, 5); /* 鸣叫50ms，停10ms， 1次 */
}
...
// {
//     static
//     if (!g_st_SysTimeFlag.bits.b1Sys10msFlag4)
//         return;

//     // if()
//     // if (g_stCellInfoReport.unMdlFault_Third.all || g_stCellInfoReport.unMdlFault_First.bits.b1IchgOcp || g_stCellInfoReport.unMdlFault_First.bits.b1IdischgOcp || g_stCellInfoReport.unMdlFault_Second.bits.b1IdischgOcp)
//     //     // BEEP_KeyTone();
//     //     BEEP_Start(5, 5, 0);
//     // else
//     //     BEEP_Stop();
...
void App_beep(void)
{
	...
	if (!g_st_SysTimeFlag.bits.b1Sys10msFlag5)
		return;
	...
	case 0:
		// if (g_stCellInfoReport.unMdlFault_Third.all || g_stCellInfoReport.unMdlFault_First.bits.b1IchgOcp || g_stCellInfoReport.unMdlFault_First.bits.b1IdischgOcp || g_stCellInfoReport.unMdlFault_Second.bits.b1IdischgOcp)
        // if(g_stCellInfoReport.unMdlFault_Third.all || System_ERROR_UserCallback(ERROR_STATUS_CBC_DSG) || )
        if(isProtect())
		{
			step = 1;
		}
	...
	case 1:
		// BEEP_Start(50, 100, 0);
		// BEEP_Start(20, 40, 0);
        BEEP_Start(100, 200, 0);

		step = 2;

		break;
	case 2:
		// if (0 == (g_stCellInfoReport.unMdlFault_Third.all || g_stCellInfoReport.unMdlFault_First.bits.b1IchgOcp || g_stCellInfoReport.unMdlFault_First.bits.b1IdischgOcp || g_stCellInfoReport.unMdlFault_Second.bits.b1IdischgOcp))
