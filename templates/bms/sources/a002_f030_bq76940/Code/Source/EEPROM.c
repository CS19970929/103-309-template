...
// Legacy EEPROM API: physical storage is MCU internal Flash only.
UINT8 WriteEEPROM_Byte(UINT16 addr, UINT8 val)
{
	...
	if ((UINT32)addr >= PROJECT_CFG_FLASH_VEEPROM_SIZE)
	{
	...
	Feed_IWatchDog;
	flash_addr = PROJECT_CFG_FLASH_VEEPROM_START + (UINT32)addr;
	half_addr = flash_addr & ~1U;
	old_half = FlashReadOneHalfWord(half_addr);

	if ((flash_addr & 1U) == 0U)
	{
		new_half = (UINT16)((old_half & 0xFF00U) | val);
	}
	else
	{
		new_half = (UINT16)((old_half & 0x00FFU) | ((UINT16)val << 8));
	}

	if (FlashWriteOneHalfWord(half_addr, new_half) != FLASH_COMPLETE)
	{
		System_ERROR_UserCallback(ERROR_EEPROM_COM);
		return 1;
	}
...
UINT8 ReadEEPROM_Byte(UINT16 addr)
{
	...
	Feed_IWatchDog;
	temp = *(volatile UINT8 *)(PROJECT_CFG_FLASH_VEEPROM_START + (UINT32)addr);
	Feed_IWatchDog;
	return temp;
}
...
UINT16 ReadEEPROM_Word_NoZone(UINT16 addr)
{
	UINT16 tmp16a;
	UINT8 tmp8a, tmp8b;
	if (((UINT32)addr + 1U) >= PROJECT_CFG_FLASH_VEEPROM_SIZE)
	{
		System_ERROR_UserCallback(ERROR_EEPROM_COM);
		return 0xFFFF;
	}
	...
	if ((addr & 1U) == 0U)
	{
		return FlashReadOneHalfWord(PROJECT_CFG_FLASH_VEEPROM_START + (UINT32)addr);
	}
...
UINT8 WriteEEPROM_Word_NoZone(UINT16 addr, UINT16 data)
{
	...
	if ((addr & 1U) == 0U)
	{
		return (FlashWriteOneHalfWord(PROJECT_CFG_FLASH_VEEPROM_START + (UINT32)addr, data) == FLASH_COMPLETE) ? 0 : 1;
	}
...
{
	return ReadEEPROM_Word_NoZone(addr);
}
...
{
	return ReadEEPROM_Byte(addr);
}

UINT8 Storage_WriteByte(UINT16 addr, UINT8 data)
{
	return WriteEEPROM_Byte(addr, data);
}
...
{
	return WriteEEPROM_Word_NoZone(addr, data);
}
...
void InitData_E2prom(void)
{
	...
	{ // 第一次上电，用于量产
		...
		WriteEEPROM_Word_WithZone(0x032c, 0xffff);
...
{
	/* Internal Flash parameter storage; no external EEPROM GPIO or 24xx driver. */
	InitData_E2prom();
}
