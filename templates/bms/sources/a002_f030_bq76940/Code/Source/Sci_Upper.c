...
void Sci_Deal_WrRegs_0x10(struct RS485MSG *s)
{
...
#ifdef __VIRTURE_CURR__
	case 0xfff3:
	{
		if (((s->u16Buffer[4] << 8) + s->u16Buffer[5]) == 1)
		{
			I_CHG = (s->u16Buffer[7] << 8) + s->u16Buffer[8];
		}
	}
	...
	case 0xfff4:
	{
		if ((s->u16Buffer[4] << 8) + s->u16Buffer[5] == 1)
		{
			I_DSG = (s->u16Buffer[7] << 8) + s->u16Buffer[8];
		}
	}
...
void Sci_WrReg_0x06_BMS_FunctionON(struct RS485MSG *s)
{
	...
	if (u16SciRegData >= 1 && u16SciRegData <= 32)
	{
		...
		case 8: // 激活模拟前端AFE1
			App_WakeUpAFE();
			InitialisebqMaximo(DEVICE_ADDR_AFE1, 1, 1);
			break;
