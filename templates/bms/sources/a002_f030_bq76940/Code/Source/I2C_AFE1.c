...
const unsigned int OVPThreshold = AFE_COV_H;
const unsigned int UVPThreshold = AFE_CUV_H;
...
int I2CSendBytes(unsigned char I2CSlaveAddress, unsigned char *DataBuffer, unsigned int ByteCount, unsigned int *SentByte)
{
	...
	i2c_SendByte1(I2CSlaveAddress << 1); // 发送AFE地址
	if (1 == i2c_WaitAck1())
	{
		*SentByte = NumberOfBytesSent;
		return 1;
	...
	for (NumberOfBytesSent = 0; NumberOfBytesSent < ByteCount; NumberOfBytesSent++)
	{
		i2c_SendByte1(*DataPointer); // 发送低地址
		if (1 == i2c_WaitAck1())
		{
			*SentByte = NumberOfBytesSent;
			return 1;
...
int I2CReadBytes(unsigned char I2CSlaveAddress, unsigned char *DataBuffer, unsigned int ExpectedByteNumber, unsigned int *NumberOfReceivedBytes)
{
	...
	*NumberOfReceivedBytesPointer = 0;
	...
	i2c_SendByte1((I2CSlaveAddress << 1) | I2C_RW_R); // 发送写命令
	if (1 == i2c_WaitAck1())
	{
	...
	for (i = 0; i < ExpectedByteNumber; i++)
	{
		if ((i + 1) < ExpectedByteNumber)
		{
			*DataPointer = i2c_ReadByte1(1);
		}
		else
		{
			*DataPointer = i2c_ReadByte1(0);
		}
		DataPointer++;
	}
#else
	...
	for (i = 0; i < ExpectedByteNumber; i++)
	{
		// *DataPointer++ = i2c_ReadByte1(); /* 读1个字节 */
		*DataPointer = i2c_ReadByte1(); /* 读1个字节 */

		/* 每读完1个字节后，需要发送Ack， 最后一个字节不需要Ack，发Nack */
		if (i != ExpectedByteNumber - 1)
		{
			i2c_Ack1(); /* 中间字节读完后，CPU产生ACK信号(驱动SDA = 0) */
		}
		else
		{
			i2c_NAck1(); /* 最后1个字节读完后，CPU产生NACK信号(驱动SDA = 1) */
		}
		DataPointer++;
	}
...
int I2CWriteBlockWithCRC(unsigned char I2CSlaveAddress, unsigned char StartAddress, unsigned char *Buffer, unsigned char Length)
{
	...
	*Pointer = CRC8(BufferCRCC, 3, CRC_KEY);
	...
	// result = I2CSendBytes2(I2CSlaveAddress, BufferCRCC + 1, 2 * Length + 1, &SentByte);
...
int InitialisebqMaximo(unsigned char I2CSlaveAddress, uint8_t chgon, uint8_t dsgon)
{
	int result = 0;
	...
	// Registers_AFE1.SysCtrl2.SysCtrl2Bit.CHG_ON = chgon; // 强制打开一下下
	// Registers_AFE1.SysCtrl2.SysCtrl2Bit.DSG_ON = dsgon; // 强制打开一下下
...
{
	bsp_InitI2C(I2C_SCL_GPIO, I2C_SCL_PIN, I2C_SDA_PIN);
}

void InitAFE1(uint8_t chgonoff, uint8_t dsgonoff)
{
	initAFE1_IIC();

	InitialisebqMaximo(DEVICE_ADDR_AFE1, chgonoff, dsgonoff);
