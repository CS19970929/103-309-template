...
- 充电接近满电：
  - `Vmax` 在 `V100 - 100 mV` 到 `V100` 之间，且 SOC < 95%，持续 10 s 后每次 +1%。
  - `Vmax >= V100` 且 SOC < 100%，根据 SOC 高低使用 8 s 或 4 s 牵引。
  - `Vmax >= V100 + 50 mV`，持续 2 s 强制 +1%。
- 放电接近空电：
  - `Vmin` 在 `V0` 到 `V0 + 100 mV` 之间，且 SOC > 5%，持续 10 s 后每次 -1%。
  - `Vmin <= V0` 且 SOC > 0%，根据 SOC 高低使用 8 s 或 4 s 牵引。
  - `Vmin <= V0 - 50 mV`，持续 2 s 强制 -1%。
...
- 充电时，`Vmax >= V100` 且 `Vmin >= SOC_FULL_CELL_MIN_MV`，持续 5 s 后直接校准到 100%，并把 `CapNow` 对齐到 `CapFull`。
- 放电时，`Vmin <= V0` 且电压不低于 2000 mV，持续 10 s 后直接校准到 0%。
- 无电流时清除满充/空放校准延时。
...
- 静置累计 300 s 后开始修正。
- 若满足满电锚点，直接置 100%；若满足空电锚点，直接置 0%。
- 否则每 60 s 查一次 OCV 表，当前 SOC 与 OCV SOC 差值超过 5% 时，只修正 1%。
