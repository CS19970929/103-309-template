...
- 不接 CAN 设备或 no-ACK 时，确认普通周期广播 TX pending 不反复阻塞 RTC idle，`Can_PrepareSleep()` 可在入睡前清队列并关闭 CMNT。
...
- `MCU_WK` 和按键当前已删除软件滤波计数；首次原始电平仍应预置，不能把开机已有电平误判为新边沿。
- 单击、连击、长按和 `MCU_WK` 插拔需上板观察无软件滤波后的抖动体验。
- 睡眠 SOC 预览。
- 长按进入 deep sleep。
- STOP 前 GPIO 泄漏。
