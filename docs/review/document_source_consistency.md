...
| `docs/FACTORY_AGING_UPGRADE_RESET_CONFIG_2026-05-26.md` | `Project_Config.h`, `EEPROM.c`, `FactoryAging.c` | CONFLICT | 旧文档口径可能仍按“不重置老化时间”理解 | 当前 `PROJECT_CFG_UPGRADE_PARAM_RESET_FACTORY_AGING_TIME 1`；`UpgradeParamPolicy_ApplyOnce()` 调用 `FactoryAging_ResetTimeByHost()`，该函数清 elapsed 后进入 `FactoryAging_EnterRunningFromHost()` | 中 | 以 `storage_design.md` 和 `storage_upgrade_gate_plan_2026-05-31.md` 为准，发布前确认老化 reset 是否符合本批升级策略 |
...
| `CAN_RTC低功耗重构说明_2026-05-14.md` | `app_lowpower.c`, `Can_HDX.c`, `RTC.c` | CONFLICT | 文档称 RTC 不再因 CAN 状态机忙推迟入睡，旧审查又曾误写 CAN bus active 会直接置 `LP_BLOCK_COMM` | 当前 `LP_BuildBlockReason()` 只因 `Sci_IsAnyPortBusy()/Can_IsBusy()` 置 `LP_BLOCK_COMM`；`Can_IsBusActive()` 不直接阻塞 STOP，主要影响 `RTC_GetWakeupPeriodSeconds()` 和 `Can_RtcWakeService()` | 中 | 归档旧结论；当前口径以 `docs/design/low_power_design.md` 和 `docs/review/low_power_comm_wake_gate_plan_2026-05-31.md` 为准 |
...
|---|---|---|---|---|---|---|
| 量产主路径虚拟电流旧结论 | `DataDeal.c:1238-1239` | CONFLICT | 部分 review/overview 文档仍写主路径调用 `test_Autocurrent_cycle()` | 当前主路径调用 `DataLoad_Current()`，`test_Autocurrent_cycle()` 为注释状态 | 中 | 修正权威文档，把剩余问题改为“是否删除或隔离虚拟电流测试入口” |
...
| CAN RTC wake service | `Can_HDX.c:1128-1182`, `RTC.c:375-424` | PARTIAL | 多份旧 CAN/低功耗文档互相冲突，尤其是 CAN busy 与 CAN bus active 的阻塞口径混用 | RTC 唤醒可短时发 CAN 周期帧；CAN busy 会阻塞进入 STOP，CAN bus active 主要调整 RTC wake period/service 策略 | 中 | 合并到 `low_power_design.md` 并标待确认 |
