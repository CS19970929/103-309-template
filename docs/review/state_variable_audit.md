...
| SV-LOG-001 | `s_log_record_flag` | `LogRecord.c:7`, `LogRecord.c:65-73`, `LogRecord.c:122-143` | startup/sleep 日志请求标志 | KEEP_BUT_REFACTOR | 可收口。当前 sleep 提交时已直接调用 `LogEvent_Record()`，但 startup 延迟到 1s 任务；先确认日志写入时序再删 |
| SV-LOG-002 | `su8_Event[]`, `su8_CBC_Temp` | `LogRecord.c:122-174` | 故障日志边沿去重和 CBC 温度变化记录 | MUST_KEEP | 真实事件边沿状态，保留 |
