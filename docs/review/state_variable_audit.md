...
| 启动运行态初始化已显式 `LedBar_Init()` | `AppInit.c:36-45` |
...
|---|---|---|---|---|---|
| SV-LED-001 | `s_ledbar.initialized` | `AppInit.c:36-45`, `LedBar.c:171-177`, `LedBar.c:1034-1067`, `LedBar.c:1263-1288` | LedBar 启动一次性初始化完成标志，同时保护外部 API 和 TIM4 ISR 早到访问 | 已处理，保留安全保护 | 已在 SV-CLEAN-02 中把 `LedBar_Init()` 收口到启动运行态初始化，并移除 `APP_LedBar()` 入口懒初始化；`LedBar_EnsureInit()` 仍保留给外部 API、STOP 前 GPIO 和 ISR/debug 防护 |
...
| SV-LED-004 | `s_ledbar.key_active`, `key_wakeup_armed`, `key_hold_10ms`, `key_press_start_10ms`, `key_long_handled` | `LedBar.c` | 按键首次原始电平预置、上升沿检测和长按判断；软件防抖计数已删除 | KEEP_BUT_REFACTOR | 本轮按试验要求删除 `key_*_filter` 计数，只保留边沿/长按必要状态；需上板观察按键抖动 |
| SV-LED-005 | `s_ledbar.mcu_wk_active` | `LedBar.c` | `MCU_WK` 首次原始电平预置和上升沿显示触发；软件防抖计数已删除 | KEEP_BUT_REFACTOR | 本轮按试验要求删除 `mcu_wk_*_filter` 计数，只保留上一拍状态；需上板观察毛刺 |
