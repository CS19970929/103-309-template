...
| RISK-RTC-IWDG-002 | `PROJECT_CFG_WDOG_ENABLE=0` 但 `Init_IWDG()` 当前无条件启动 | `Project_Config.h`, `AppInit.c`, `System_Init.c` | 宏含义与实际行为不一致，RTC wake 安全窗口判断也可能不一致 | CONFLICT | 先确认量产 IWDG 策略，再统一宏、函数和 build guard |
