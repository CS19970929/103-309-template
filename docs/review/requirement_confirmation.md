...
| REQ-SYS-003 | 量产 target 不得带调试/测试符号 | `uvprojx` 中 `FD_Release` defines、`tools/project_check.py`、`Project_BuildGuard.h` | Keil 工程、`tools/project_check.py`、BuildGuard | `FD_Release` 不定义 `_DEBUG_` / `PROJECT_CFG_DEBUG_WATCH_ENABLE` / `PROJECT_CFG_DEBUG_MONITOR_ENABLE` / `PROJECT_CFG_IRQ_DEBUG_ENABLE` / `PROJECT_CFG_IRQ_DEBUG_EVENT_ENABLE` / `ELOG_OUTPUT_ENABLE` / `FLASH64K_APP_*`，且不覆盖非 0 build profile | 间接 | 间接 | 是 | 间接 | 是 | MUST_KEEP；测试功能必须隔离 |
...
| REQ-SOC-005 | 低压尾段必须通过 V0 和 empty tail 限制 SOC 虚高；mid-tail 已删除 | `Project_Config.h`, `SocEnhance.c` | `SocEnhance.c` | low-tail target、empty fast、display low | 是 | 是 | 是 | 是 | 是 | MUST_KEEP low-tail；mid-tail 已按确认删除 |
...
| REQ-SOC-010 | 上位机可写 SOC 容量参数和一次性设置 SOC 应触发指定动作 | `SocEnhance.c`, `Sci_Upper.c` | `SocEnhance.c`, `Sci_Upper.c` | flag 2 capacity reset、flag 3 one-shot set SOC；flag 1 手动 OCV 已删除 | 是 | 是 | 是 | 否 | 是 | MUST_KEEP capacity reset/set once；runtime table/manual OCV 已删除 |
...
| REQ-SV-005 | 按键、`MCU_WK`、SOC sample seq、AFE fault 计数等真实历史状态必须保留 | `LedBar.c`, `SOC.c:116-142`, `DataDeal.c:825-917` | LedBar 已按试验要求删除 key/`MCU_WK` 软件防抖计数，只保留上一拍原始电平、边沿和长按必要状态；SOC sample seq、AFE fault 计数仍保留 | 误删边沿/长按状态会造成显示触发或长按失效；删除滤波后可能带来按键抖动或 `MCU_WK` 毛刺体验问题 | KEEP_BUT_REFACTOR | 是否接受无软件滤波试验结果，还是后续恢复最小防抖？ | 本轮先试删滤波；上板观察后决定是否恢复 | 待上板确认 |
