...
`读取BMS日志` 不再一次性读取 `0xC008` 起 100 条记录。UI 会按 20 条一组分 5 次读取，每组之间等待 120ms；任意一组返回 `BMS_ERROR`、超时或响应长度异常时，自动等待 300ms 后重试，最多重试 3 次。这样可以避免 BMS App 串口兼容层短暂忙碌、CAN 总线有其它设备或 100 帧连续块读造成的偶发失败。
