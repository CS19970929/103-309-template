...
- [x] 主机回放：执行 `python3 tools/soc_replay_test.py`，覆盖冷启动、快照恢复、设置一次 SOC、积分、循环/SOH、满电确认、低压表、mid-tail 已删除、RTC 静置、久置低 OCV 慢速下修、显示平滑、Fixed/Zero 覆盖和自动校准步长。
...
- [ ] 小电流死区：`Ichg/Idsg <0.2A` 时不积分。
- [ ] 样本去重：AFE 电流样本序号不变时，`App_SOC()` 不重复积分旧电流。
- [ ] 充放电方向切换：方向切换时积分余量不反向继承。
- [ ] 放电循环累计：每累计出厂容量 `1%` 的放电量，内部 `cycle_x100 + 1`。
- [ ] SOH 映射：确认 `SOH = clamp(100 - cycle / 100, 80, 100)`，并用 SOH 反推 `cap_full`。
- [ ] 设置一次 SOC：上位机 `0x1005` 后，内部 SOC、`CapNow` 和对外发布 SOC 同步到设定值。
- [ ] 历史 `SOC_Fixed` / `SOC_Zero`：确认写入后不再覆盖 SOC 发布值，也不破坏内部容量、循环和 Flash 快照。

## 满电验证

- [ ] 满电确认：非 `DSG`、`VCellMax >= V100 - 80mV` 后，满足 `VCellMin >= V100 - 30mV` 且压差 `<=120mV` 持续 `5s`，或内部 SOC `>=95%`、`VCellMin >= V100 - 80mV`、压差 `<=120mV` 累计 `15s` 后，每次只上修 `1%` 直到 `100%`。
...
- [ ] 自动校准步长：满电确认、低压表、静置/RTC OCV 每次内部 SOC 校准变化都不得超过 `1%`。
- [ ] 宽范围低压表：非充电下 `VCellMin <=3400mV` 后，按 RELAX、轻载 `<=C/5`、中载 `<=C/2`、重载 `>C/2` 四档表驱动下修。
- [ ] 大电流 holdoff：`Idsg > C/2` 或刚结束大电流后的 `30s` 内，若 `VCellMin > V0 + 50mV`，低压表和静置 OCV 都不得校准 SOC。
...
- [ ] Mid-tail 删除：源码无 `s_mid_tail_table`、`mid_ticks`、`u8MidTailActive` 和相关测试模型；中段电压不触发独立 mid-tail 下修。
- [ ] 静置 OCV：短静置不锁存 deferred target，后续 active 充/放电不消化隐藏目标；连续稳定长静置约 60 分钟量级首次 `1%` 慢速下修。
- [ ] RTC OCV：长时间休眠唤醒后只推进长静置慢速下修，不额外扣 RTC 自耗，不锁存短静置 deferred target，不产生大幅显示跳变。

## 异常输入验证

- [ ] 电压无效：模拟 `VCellMax < VCellMin`、单体低于 `2000mV`、单体高于 `5000mV`、压差高于 `1000mV`，确认 OCV、满电确认、低压表均不动作。
