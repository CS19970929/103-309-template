...
原始主项目和 AFE 芯片的 SPI 通信存在明确问题，不能认为是可靠闭环。本分支已按官方例程修复底层帧校验、重试和上层错误传播，并完成一次 ST-Link/OpenOCD 板上验证。
...
- 原协议层存在确定 bug：写寄存器 ACK 读取位置错误，软件复位少发最后一个 dummy 字节，读寄存器没有完整校验回显字段；本分支已修复。
- 原上层错误处理不完整：采样函数忽略所有 SPI 读返回值，初始化写寄存器不检查返回值，也没有官方例程的重试、寄存器读回校验和 SPI 错误闭环；本分支已让采样失败进入 SPI 错误路径。
- 原 CS 初始状态有风险：`InitIO()` 只把 CS 配成输出，没有拉高；本分支已在软件 SPI 初始化和每帧开始前保证 CS 空闲高电平。

所以这次处理后的判断是：底层 SH3673520 SPI 帧格式已经与官方例程一致，板上能读回 AFE 配置和电芯电压；剩余重点是继续清理 Keil 编译警告、统一 Flash/MCU 配置和补长期稳定性测试。
...
- `sh36735_write_reg_u8()` 改为完整 5 字节写帧校验：`0xFF, 0x01, reg, data, 0xA5`。
- `sh36735_read_regs()` 改为校验 `0xFF, 0x02, reg, len, data..., crc`，失败时不覆盖调用方缓冲区。
- `sh36735_sw_reset()` 补齐最后一个 dummy 字节并完整校验回包。
- SPI 读写增加 5 次重试，并统一维护 `sys_time.crc_err`。
- `UpdateVoltageFromBqMaximo()` 改为按寄存器块返回错误 bitmask，失败时上报 `ERROR_SPI` 并阻止 `App_AFEGet()` 继续消费旧/半更新数据。
- AFE 初始化集中到 `InitAFE3520_Registers()`，关键写操作检查返回值，并读回 `SCONF4`。
- 均衡写 AFE 寄存器改为检查返回值，软件均衡状态只在写入成功后更新。
- 删除未集成的 `100ask driver`、旧 `Balance.c`、`todo.c` 等测试/废弃文件。

### 10.2 ST-Link 验证结果

- Keil 工程能生成 AXF/BIN：`0 Error(s), 50 Warning(s)`；50 个 warning 属于后续遗留清理项。
- OpenOCD 识别目标：ST-LINK V2J37S7，目标电压约 3.27 V，Cortex-M3，Flash size 128 KiB。
- APP 链接地址是 `0x08004800`，不能把 APP BIN 直接烧到 `0x08000000`；`0x08000000` 是 IAP/Bootloader 区。
- 使用 OpenOCD 将 APP BIN 写入 `0x08004800`，手动设置 `VTOR=0x08004800`、`MSP=0x20001C78`、`PC=0x0800494C` 后，能命中 `main()` 并继续运行。
- 运行后读取 `sys_time.crc_err=0`、`System_ErrFlag.u8ErrFlag_Com_SPI=0`。
- 读回 `Registers_AFE1`：`SCONF4=0x13`，与 19 串配置一致。
- `g_stCellInfoReport.u16VCell[]` 已出现合理电芯电压，例如 `0x0EF5/0x0ECC/0x0EC3/...`，说明当前 AFE SPI 读通且数据进入主业务结构。

### 10.3 仍需注意

- 当前板子/工程实际按 128 KiB Flash 识别，原 `0x0803E000/0x0803E800` 超出范围，本分支已改为 `0x0801F000/0x0801F400`。
- `0x08000000` 的 IAP/Bootloader 区需要单独恢复或确认；本分支未推送、未提交启动区镜像。
- 还没有做 1000 次连续 SPI 读写统计、断线错误注入和 MOS/均衡实物动作验证。
