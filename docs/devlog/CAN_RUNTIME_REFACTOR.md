...
- `Can_HDX.c`：负责 CAN 初始化、TX queue、固定周期调度、App 命令、read-block stream、IAP 延迟入口和睡前 CMNT 关闭。
...
- `s_runtime`：周期调度 tick、1000ms/5000ms 上次调度 tick、调度初始化标志。
...
- 运行态 active/probe/no-ACK 软件退避已删除：不再保留 `bus_active`、`last_bus_activity_tick`、`no_ack_cnt`、`probe_active`、`last_probe_tick`。
