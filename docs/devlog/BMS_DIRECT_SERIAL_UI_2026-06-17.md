...
- 直连串口默认参数：
  - UI 默认波特率：`115200`（用于经 comm tool UART1→USART2 透传）；PC 直接接 BMS App/IAP 串口时手工改为 `19200`
  - Modbus 地址：`1`
...
上述能力在直连模式不可用；依赖 comm tool 的操作会提示切换到 `comm tool/CAN桥`，缓存信息则显示为不可用。

直连模式当前也支持完整老化操作：

- 开启/停止：写 `0x1102=7` / `0x1103=7`。
- 重置累计时间：写 `0x1008=0x005A`。
- 修改总时长：写 `0x1009=hours`，范围 `1..168`。
- 读取状态：读 `0xC080` 起 5 words，得到状态、剩余分钟、剩余秒和总时长。

桥接模式的剩余时间仍来自 `0x14F80208` CAN 广播。
