...
4. 在 `参数设置` 页点击 `读取保护参数`、`读取其它参数` 或 `一键读取`，SH309 保护页会按旧串口上位机布局显示。
...
```powershell
py -3.9 tools\comm_tool_host.py bms-read --port COM4 --baud 115200 --address 0x2400 --count 24 --long-timeout 15
```

读取 SH309 MOS 过温参数：

```powershell
py -3.9 tools\comm_tool_host.py bms-read --port COM4 --baud 115200 --address 0x2132 --count 5 --long-timeout 15
```

写单个参数示例：

```powershell
py -3.9 tools\comm_tool_host.py bms-write --port COM4 --baud 115200 --address 0x2400 4250 --long-timeout 15
```
...
写入仍会检查寄存器地址、数量、范围和副作用。当前 SH367309/SH309 项目中，UI 的保护页按旧串口上位机实现拆成两段读取：

- `0x2400..0x2417`：SH309 AFE 保护点，24 个寄存器。电流按 `A*10` 存储，温度按 `(℃+40)*10` 存储，电压为 mV，延时按旧上位机规则显示为 ms。
- `0x2132..0x2136`：MOS 过温保护组，5 个寄存器。前 4 项温度按 `(℃+40)*10` 存储，第 5 项为 `10ms` 单位延时。
- `0x2300` 起：其它参数。

固件侧 `Sci_WrRegs_0x10_Protect()` 和 `Sci_WrReg_0x06_Reset_ProtectElement()` 必须保持启用；否则 `0x2132` MOS 过温组写入和 `0x1002` 保护点重置不会真正生效。
...
- PC 到 comm tool 写固件缓存：`FW_DATA` 数据分块 `496 bytes`，实际 payload 为 `4 + 496 bytes`，低于协议 `512 bytes` 上限。
- BMS App 块读回复：每 `10ms` 返回 1 个寄存器帧，88 个寄存器约 `0.9s` 完成。
- CAN-IAP 升级：仍保持 `256 bytes` 块提交。该值优先保证无流控数据突发稳定性，放大到 `512/1024 bytes` 前必须做连续升级和断电恢复测试。
