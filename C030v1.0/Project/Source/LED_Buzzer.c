...
void LED_SOC(void)
{
	...
	if (g_stCellInfoReport.SocElement.u16Soc == 0)
		idx = 0;
	else if (g_stCellInfoReport.SocElement.u16Soc <= 20)
		idx = 1;
	else if (g_stCellInfoReport.SocElement.u16Soc <= 40)
		idx = 2;
	else if (g_stCellInfoReport.SocElement.u16Soc <= 60)
		idx = 3;
	else if (g_stCellInfoReport.SocElement.u16Soc <= 80)
		idx = 4;
...
void LED_ALM(void)
{
	...
	if (g_stCellInfoReport.unMdlFault_Third.all & 0x27F0 || System_ErrFlag.u8ErrFlag_CBC_DSG || System_ErrFlag.u8ErrFlag_Com_AFE1 || System_ErrFlag.u8ErrFlag_Com_EEPROM)
	{
		u8BarFlag[e_LED_ALM] = LB_LED_ON;
	}
	// 除过压低压报警，其它报警都闪
	else if ((g_stCellInfoReport.unMdlFault_First.all & 0x27F0) || (g_stCellInfoReport.unMdlFault_Second.all & 0x27F0))
	{
