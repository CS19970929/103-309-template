...
| `App_AFEGet()` | 主循环每轮调用 | 依赖 `g_200ms_task_flag` | bool flag 可能合并多个 200ms 周期 |
...
当前 AFE 任务由 `g_200ms_task_flag` 触发。该变量是 bool 语义：
...
| 旧 `SleepDeal.c` | 仍承担部分休眠标志、启动休眠和旧路径逻辑 |

问题：

1. `g_sleepModeSelect` 和 `Sleep_Mode` 同时存在，模式选择有两套状态。
...
本次已完成第一步：在 `main.c` 内新增 `Runtime_RunOnce()` 和 `Runtime_RunDebugOnce()`，主循环只调用这两个入口。任务顺序、周期、CAN/RTC/SOC 业务逻辑保持不变；后续再决定是否拆出独立 `Runtime.c/.h` 并同步 Keil 工程文件。
...
1. 用 elapsed 计数替代 `g_200ms_task_flag`。
...
2. `Sleep_Mode` 强制位改成 PowerMgr 内部兼容层，不再由多个模块直接仲裁。
3. `SleepDeal_Continue()` 改成 `PowerMgr_CommitResetSleep(mode)`，只负责旧 reset sleep 提交。
...
1. 删除不再使用的旧 `SleepDeal` 仲裁分支。
2. 删除重复的 `App_SleepDeal()` 路径或改成兼容 wrapper。
3. 更新低功耗、CAN、系统时基、README 文档。
4. 在 `tools/project_check.py` 增加静态检查：
   - 禁止 App 裸写 `Sleep_Mode`。
...
完成阶段 1 后，再做阶段 2，把 `g_200ms_task_flag` 改成 elapsed counter，并把 AFE/SOC/ADC 的调用关系固定下来。
