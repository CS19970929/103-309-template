...
- `AfeService_Init()`：启动时完成 startup zero 准备、调用 `InitAFE1()`、按老化状态设置 MOS/factory mode、完成 startup zero 或打开 CTL。
- `AfeService_Recover()`：运行中 AFE 恢复时复用同一启动策略。
...
main()
  -> InitDevice()
       ...
       AfeService_Init()// PROJECT_FEATURE_AFE
