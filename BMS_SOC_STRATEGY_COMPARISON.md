...
| 避免满电虚高 | 满电需要单体满足满电窗口并持续约 60s，同时要求 SOC/容量接近满或 taper 电流低于 `C/20` | 不依赖充电器恒压尾段，充电早期高电压也不会立即显示 100% |
...
| 避免显示跳变 | 内部 SOC 与对外 `DisplaySoc` 分离，正常约 `1%/s` 跟随；在线 OCV 只在轻载、电压稳定、非重载恢复窗口下按 `1%/30s` 小步修正 | 数码管、CAN、RS485 读数更稳定 |
