...
4. `Flash` 快照保存恢复体验所需的 SOC、容量、循环累计和回弹保护标志，但不把每个小数积分都写入 Flash。
5. `SOH` 当前按循环次数映射，不做运行中 `FCC` 学习；容量基准保持可解释、可回放。
...
| --- | --- | --- | --- | --- |
| 电压/OCV 查表 | 静置后按单体电压查 SOC 表 | 简单、资源占用低 | 负载下误差大，`LFP` 平台区分辨率差，容易跳变 | 只用于手动 OCV 刷新、RTC 唤醒、长静置小幅校准；普通启动优先用 Flash 快照或默认 60% |
...
| 目标 | 实现点 | 用户体验收益 |
| --- | --- | --- |
| 避免 SOC 抖动 | `CHG / DSG / RELAX` 按 `0.2A` 有效电流门槛判定，小电流死区不积分；静置可信度另由电压稳定窗口累计 | 插拔负载、采样噪声不会导致 SOC 方向反复变化 |
| 避免满电虚高 | 满电需要非放电模式、单体满足 `V100` 相关窗口、压差满足限制，并持续快速/普通确认时间；每次最多向 `100%` 推进 `1%` | 充电早期高电压不会立即显示 100%，但真实满电后能逐步到 100% |
| 避免低压风险被高 SOC 掩盖 | 中低压弱约束和低压尾段表按 `V0` 相对电压、电流档位、收敛周期限制 SOC 上限 | 用户在低压临界区看到更保守、更安全的 SOC |
| 避免异常采样误校准 | 异常单体电压、反序压差、三级保护故障、AFE/ADC/CBC/温度异常均禁止电压类校准 | 采样故障或保护态不会把 SOC 错误拉高/拉低 |
| 避免显示跳变 | 内部 SOC 与对外 `DisplaySoc` 分离，普通场景约 `5s/1%` 跟随；低压末端可加快到 `1s/1%` 或 `200ms/1%` | 数码管、CAN、RS485 读数更稳定，低压末端又不会长期虚高 |
| 保持冷启动可信 | `Flash` SOC 快照 V2 保存 `SOC/CapNow/CapFull/Cycle/DsgAcc/Flags`，兼容 V1；重载后重启保留 rebound holdoff | 升级或休眠唤醒后不丢现场 SOC，未回弹电压不会立即把 SOC 校低 |
| 降低 SOH 漂移 | `SOH = clamp(100 - cycle/100, 80, 100)`，按循环次数线性映射，不做现场容量学习 | 长期行为简单、可解释，避免学习状态误触发造成容量基准漂移 |
