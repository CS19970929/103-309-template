...
| 参数 | 建议值 |
| --- | --- |
| 波特率 | 当前 F103C8 适配层默认 `115200`；硬件时钟确认后可升级到 `460800` 或 `921600` |
| 数据位 | 8 |
| 校验 | None |
| 停止位 | 1 |
...
1. 独立 IAP 构建脚本：`tools/build_iap_bootloader.ps1`。
2. CAN 升级协议实现。
3. Flash 擦写和边界保护。
4. CRC 校验和失败留在 IAP。
...
5. 固件升级状态机。
6. 错误日志和进度上报。
7. STM32F103C8 适配层和构建脚本：`upgrader_mcu/stm32f103c8`、`tools/build_upgrader_mcu_f103c8.ps1`。
