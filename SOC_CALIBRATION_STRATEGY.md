...
“每次最多 `1%`”是硬约束：满电确认、低压收敛、静置/RTC OCV、骑行中电压表修正都不能一次性跳变超过 `1%`。稳定静置/RTC OCV 默认只记录可信目标，不立即追平；后续由方向匹配的充电或放电阶段按小步消化。启动初始化、上位机 `0x1005` 设置一次 SOC、`SOC_Fixed/SOC_Zero` 显示覆盖不属于运行态自动校准。
...
| 静置稳定 OCV | `RELAX` 下电压稳定 `5min` 后，按 `10min/step` 节拍 | 先记录 OCV deferred target；方向匹配的充/放电阶段按 `10min/1%` 消化 | 静置不立刻追平，减少停车跳变 |
| 久置低 OCV | 稳定静置超过 `30min`，且 OCV target 低于内部 SOC | 静置阶段按 `30min/1%` 慢速下修 | 避免电池久置自放电后 SOC 长期虚高 |
