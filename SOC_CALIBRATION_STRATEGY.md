...
```text
安时积分主导 + 满电确认 + 中低压弱约束 + 低压表收敛 + 静置/RTC OCV 小步修正 + 显示平滑
```
...
该表不应被理解为“所有电池都准确”的绝对真值。不同批次、温度、老化和内阻都会改变端电压关系，所以当前策略只把 OCV 表用于可信场景的小步修正；骑行中只做上限约束，不做双向强融合。
...
| 自动满电、自动空电、中低压弱约束、静置/RTC OCV | 每次校准 `1%`，显示继续平滑 |
