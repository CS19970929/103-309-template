...
1. 默认 `V100 - 80mV` 和压差 `120mV` 适合先保守验证。
2. 如果真实充满后长期不能到 100%，先确认 charger taper 电流能低于 `C/20`，再考虑把 margin 从 `80mV` 增到 `100~120mV`。
3. 如果早早显示 100%，先把 margin 降到 `50~60mV`，或把压差从 `120mV` 降到 `80~100mV`。
4. 不建议把压差检查关掉，除非有独立均衡/采样证明。
