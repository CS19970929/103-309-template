...
当前工程开启了 `wdog_enable`，而 `__FUNC_RTC__` 模式下 IWDG 超时时间约为 26 秒。

如果 RTC 单次 STOP 周期直接按 3 分钟执行，系统会先被 IWDG 复位，根本等不到 RTC 正常唤醒。为避免这个硬冲突，`RTC_GetWakeupPeriodSeconds()` 现在还会在看门狗开启时自动压缩到安全窗口内。

当前策略：

- 保留 `u16Sleep_TimeRTC` 作为“目标休眠周期”配置来源
- 实际单次 STOP 周期自动限制在 IWDG 安全窗口内
- `rtc_sleep` 的休眠时长累计与 SOC 静置补偿同步使用这个实际周期

结果：即使配置了分钟级静置周期，系统也会以更短的 RTC 节拍分段睡眠，从而既不被看门狗提前复位，又能持续做低功耗巡检
...
此前 RTC 周期唤醒只做了薄检测，主要依赖：

- 电芯电压刷新
- 电流存在性判断
- MOS 开关状态判断

这对“检测是否有活动”足够，但对保护状态同步和低功耗期间的数据新鲜度不够。

现在补充了两点：

- `sh36xx` 路径下，RTC 唤醒时同步刷新温度数据
- 读取 AFE 状态后立刻执行 `Fault_ChangeToMCU()`，把 AFE 硬件保护状态同步到 MCU 的 fault 视图；一旦存在三段故障位，就退出 HICCUP 继续正常运行链路

这意味着当前项目在低功耗期间不再只是“看有无电流/电压越界”，而是会把 AFE 的硬件保护状态纳入周期唤醒判定
