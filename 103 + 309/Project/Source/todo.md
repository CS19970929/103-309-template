...
完善全程rtc，只有低压深度休眠，备份域记录电池运行时间，32bit，完善读写全局时间，升级默认不重置时间，完善rtc时间，增加flash保存，
备份域1s一记，flash默认10 min记录，待评估擦写均衡，升级默认不重置时间

增加rtc时钟功能，上位机可设置、读写rtc时间，增加2个备份域寄存器用于记录电池运行时间，通过上位机设置的初始rtc时间和电池全局运行时间来模拟rtc时钟，rtc休眠期间也需要记录、存储电池运行时间到备份域、flash，正常运行时，备份域1s一记，flash默认10 min记录，待评估擦写均衡，rtc期间电池运行时间记录取决于rtc唤醒时间，直接定位rtc_sleep函数，先给出plan

stm32f0和f1进入stop后，rtc是否还有用，功耗怎么样,
能否使用stm32自带rtc实现,而不是目前的软件模拟，进入DEEP_MODE后，不进行rtc唤醒，stm32的rtc模块还是否有用

mcu进入深度休眠，不会rtc周期唤醒，rtc时间还会增加吗，mcu还会触发rtc sec中断？
完成本次任务后，连同todo.md一起提交git，并推送到远端