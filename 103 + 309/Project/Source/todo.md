...
请完整梳理当前 STM32 BMS 项目，并输出中文项目文档。

重要约束：
1. 只允许分析和生成文档，不要修改业务代码
2. 不要引入 HAL，不要重构，不要格式化源码
3. 不要猜测，不确定的内容必须标记为“待确认”
4. 所有结论尽量标注来源文件、函数名、宏定义或数据结构
5. 重点关注 BMS 安全逻辑，不要只做表面目录总结
...
4. 模块之间的数据流
5. 关键全局变量和结构体
6. 编译、下载、Flash 地址相关配置

第三阶段：生成文档
请在项目根目录创建 docs/project-review/ 目录，并生成以下 Markdown 文件：
