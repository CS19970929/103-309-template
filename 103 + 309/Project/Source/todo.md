...
待考虑通用adc框架，目前每次修改太麻烦了
adc需求：所有项目都会采一个mos温度，偶尔项目需要adc采一些其他功能，例如总压、电流等等，需要考虑一个通用方便配置的adc框架模板，方便配置不同项目
...
你现在是嵌入式 BMS 功能安全与 Telink TLSR8251/TLSR825x 裸机工程专家。

请先不要直接修改代码，先完整阅读当前 Telink 8251 BMS 工程源码，并参考以下两份文档：

1. ST+IEC60335+Class+B软件库介绍.pdf
2. XXX-BMS 软件验证规范.docx

目标：
在 Telink 8251 BMS 工程中实现一套“BMS 软件验证 + Class B 思想的软件自检 + 故障诊断 + 故障注入测试框架”。

重点说明：
- 不要照搬 STM32 Class B 库代码，因为当前平台是 Telink 8251 / TLSR825x，不是 STM32。
- 只参考 Class B 的设计思想：启动自检、运行时周期自检、CPU/Flash/RAM/时钟/看门狗/控制流/堆栈边界等诊断思想。
- 需要结合 Telink 8251 的实际 SDK、启动文件、编译链路、Flash 布局、RAM 布局、定时器、看门狗、BLE/OTA、主循环结构来设计。
- 当前工程是 BMS 工程，安全优先。任何自检或诊断失败，必须进入明确的 Fail Safe 流程：关闭充电 MOS、关闭放电 MOS、停止危险输出、记录故障码、上报故障状态，并根据现有工程机制决定是否复位或保持故障锁定。
- 所有代码必须适合裸机工程，不引入 RTOS。
- 不要破坏现有 BLE、OTA、低功耗、BMS 保护逻辑。
- 不要做过度设计，优先小步可验证实现。
...
- 当前工程已有能力
- 当前工程缺失能力
- 不确定点列表，必须标注需要人工确认的地方

====================
二、把验证规范转换成代码需求
====================

请根据 XXX-BMS 软件验证规范.docx 中的软件验证项目，建立“测试项 -> 软件诊断/保护逻辑 -> 代码位置 -> 测试方法”的映射表。

至少覆盖以下项目：

SW-VD-001 Cell 过压保护
SW-VD-002 Cell 欠压保护/报告
SW-VD-003 充电过流保护
SW-VD-004 放电过流保护
SW-VD-005 充电过温保护
SW-VD-006 充电低温保护
...
SW-VD-008 放电低温保护
SW-VD-009 AFE 失效保护
SW-VD-010 AFE 通信失效保护
SW-VD-011 Cell 电压采样断线保护
SW-VD-012 NTC 采样断线保护
SW-VD-013 电流采样电阻漂移失效保护
SW-VD-014 一级关断电路失效保护
SW-VD-015 电流采样电路失效保护
SW-VD-016 单片机测试

每一项都要给出：
- 现有代码是否已经实现
- 如果已实现，指出文件、函数、关键变量
- 如果未实现，建议新增的文件、函数、数据结构
- 触发条件
- 恢复条件
- 滤波/持续时间
- 故障码
...
====================
三、Class B 思想在 Telink 8251 上的落地方案
====================

请参考 ST Class B 文档，但要改造成 Telink 8251 可实现方案。

需要设计以下模块：

1. 启动自检 BMS_SelfTest_Startup()
   内容包括：
   - Watchdog 基本自检：确认看门狗能配置、能喂狗；不要在正常固件里故意等待复位，避免影响用户体验。若要验证复位能力，设计成受测试宏控制。
   - CPU 寄存器简化自检：根据 tc32/telink 编译器可行性决定使用 C 实现还是汇编实现。先实现低风险 C 版本，再评估是否补充汇编。
   - Flash 完整性 CRC：编译后计算固件 CRC，存储在 Flash 固定位置或固件尾部；启动时计算应用区 CRC 并比对。
   - RAM 启动自检：对非栈、非全局关键运行区做 March C 或 checkerboard 测试；不能破坏已初始化数据、栈、BLE 协议栈 RAM。
   - 系统时钟自检：用一个相对独立的计时源或 Telink SDK 可用 API 检查系统 tick/定时器频率是否在容差范围内。
   - 控制流自检：启动流程关键节点打点，最终比对签名。
   - 栈边界初始化：在栈低地址保护区写入 magic pattern。

2. 运行时周期自检 BMS_SelfTest_RuntimeTask()
   内容包括：
   - 局部 CPU 自检
   - 栈边界检测
   - 周期 Flash 分块 CRC
   - 关键 RAM 区域分块 March/反码校验
   - ClassB 变量双反存储校验
   - 系统时钟周期检测
   - 控制流监控
   - 主循环卡死/任务卡死监控
   - 中断过频/中断丢失监控
   - AFE 通信生命计数监控
   - BLE/OTA 状态下的自检调度降载，避免影响 OTA

3. Fail Safe 处理 BMS_FailSafe_Enter(reason)
   要求：
   - 关闭 Charge MOS
   - 关闭 Discharge MOS
   - 停止预充/均衡/加热等危险输出
   ...
   - 记录复位/故障原因
   - 对外上报故障
   - 可配置是否立即复位、延时复位、锁定等待掉电、允许充电器恢复
   - 不能在中断里做复杂操作，中断里只置位故障，主循环执行 Fail Safe

4. 故障注入框架
   需要支持编译期开关：
   - BMS_DIAG_SELFTEST_ENABLE
   - BMS_DIAG_FAULT_INJECT_ENABLE
   - BMS_DIAG_TEST_CMD_ENABLE
   - BMS_DIAG_VERBOSE_ENABLE
   ...
   注入接口可以优先通过已有串口/BLE 调试命令实现。如果当前工程没有命令框架，请先设计最小测试接口，不要大改通信协议。
...
bms_diag_fault.h / .c
bms_selftest.h / .c
bms_selftest_flash_crc.c
bms_selftest_ram.c
bms_selftest_cpu.c
bms_selftest_clock.c
bms_selftest_control_flow.c
bms_selftest_stack.c
bms_fault_inject.h / .c
bms_validation_test.h / .c

如果工程已有类似模块，请优先复用，不要重复造轮子。

====================
五、Flash CRC 编译后方案
====================

请重点分析 Telink 工程是否可以实现：

1. 编译时或 post-build 阶段计算整个应用区 Flash CRC
2. 把 CRC 写入固件末尾固定结构体
3. 固件启动时读取该结构体
4. 计算应用区 CRC
5. 比对失败进入 Fail Safe
...
请输出：
- 当前工程生成的 bin/elf 文件路径
- 当前 post-build 脚本位置
- 是否已有 OTA bin 检查工具
- 如何避免 CRC 覆盖 OTA header、bootloader、NV 参数区、MAC 地址区、校准区
- 推荐的 CRC 结构体格式，例如：
...
# Telink 8251 BMS 软件验证与自检实现 Plan
...
- 通信/上报模块：
- Flash/OTA 模块：
- Watchdog/Timer/Clock 模块：

## 2. 文档需求映射表
用表格输出：
| 测试编号 | 验证项目 | 现有实现 | 缺失点 | 建议代码位置 | 触发条件 | 动作 | 测试注入方式 |

## 3. Class B 思想适配到 Telink 8251 的方案
分启动自检、运行时自检、Fail Safe、故障注入说明。

## 4. 分阶段实施计划
阶段 0：只读分析，不改代码
阶段 1：新增诊断框架和故障码，不影响业务
阶段 2：接入 BMS 保护类验证项
阶段 3：接入 AFE/采样/MOS 失效诊断
阶段 4：接入 MCU 自检：Flash/RAM/Clock/Stack/Control Flow/Watchdog
阶段 5：接入测试注入命令
阶段 6：静态分析、编译、回归测试
...
## 5. 需要人工确认的问题
例如：
- Telink 8251 Flash 实际大小
- Bootloader/OTA 占用地址
- NV 参数区地址
- MAC/校准数据区地址
- 栈起始地址和大小
- 是否允许自检失败后立即复位
- 充电器插入时 Fail Safe 后是否允许恢复
- 是否已有硬件二级保护
- MOS 是否有反馈采样
- AFE 型号和通信 CRC 支持情况
...
## 6. 不建议做的事情
明确说明：
- 不要直接移植 STM32 Class B 源码
- 不要在正常启动时故意等待 watchdog 复位
- 不要全 RAM 破坏性测试影响 BLE 协议栈
- 不要在中断里关闭复杂业务
- 不要在 OTA 过程中做高耗时 Flash 全量 CRC
- 不要破坏现有协议字段
- 不要把测试注入接口开放到正式版本

## 7. 第一批最小改动建议
请给出最小可落地版本：
- bms_diag_fault 故障码框架
- bms_fail_safe 统一安全关断
- AFE 通信失效计数
- Cell/NTC/Current/MOS 失效诊断骨架
- stack magic 检测
- control flow 轻量签名
- Flash CRC 方案先只输出脚本计划，不直接改

最后，请等待我确认 plan 后，再开始改代码。
...
优化当前分支，将所有后64K flash使用资源全部移到前64K flash中，当前已使用o2优化，flash大大减小，以后不使用后64Kflash了，同时要保证flash存储soc、日志的可靠性与寿命，日志存储也需要扩展到500条
