...
todo
测试休眠，保底休眠、过放休眠，休眠参数更改测试
can通讯唤醒

重刷代码，工厂模式测试

优化soc校准逻辑，当校准得到soc与当前soc误差保持在3以内后，就不进行soc校准，只要能保证在放电阶段、放电末端把soc体验做好，这种体验是否更好，同时修改soc skill，永远不允许静止soc向上校准

优化can架构？每次发送完后，can进入休眠模式能否更好，busoff使用自动恢复而不是软件恢复是否更好,Can_BusOFF_Monitor