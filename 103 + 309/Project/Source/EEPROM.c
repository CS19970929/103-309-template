...
static void EEPROM_BuildRWParamData(STORAGE_FLASH_RW_PARAM_DATA *data)
{
	...
	for (i = 0U; i < E2P_PARA_NUM_RESERVED_RW_PARAM; ++i)
	{
...
static void EEPROM_LoadRWParametersFromFlash(void)
{
	...
	/* Runtime defaults are already loaded. A missing, corrupt or old-format
	 * record is repaired by writing those defaults in the current format. */
...
#else
	UINT8 result = 1U;
	UINT8 rw_param_dirty = 0U;

#if (!UPGRADE_PARAM_FORCE_REAPPLY)
	if (StorageFlash_GetConfigPolicyVersion() == UPGRADE_PARAM_POLICY_VERSION)
	{
		return 1U;
	}
#endif
