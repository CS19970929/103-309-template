...
上位机客户版和完整版的“事件日志”页自动适配100/500条，无需选择。每次读取先读100条，再尝试第二页；仅第二页返回功能0x03、错误码0x01（本固件地址非法）才识别为100条。500条按每帧100字读取 `0xC008/0xC06C/0xC0D0/0xC134/0xC198`。超时、CRC错误（本协议0x02）、短响应及其他页面错误均报错，不将通信失败误判为100条。顺序仍为最新在前，事件/间隔字节含义不变，全部成功才替换界面数据；空记录不影响容量判断。多帧期间设备仍可能新增记录，原协议没有冻结快照机制。
...
- 客户版目录：`BmsTool.Windows/publish/customer-win-x64-log-auto-20260907/`
- 完整版目录：`BmsFactoryTest.Windows/publish/internal-full-win-x64-log-auto-20260907/`
