...
#define E2P_ADDR_E2POS_AFE_Parameters			3000		//�?974，AFE参数保存到eeprom的起始地址
...
#define AFE_UTC           ((-20 + 40) * 10)
#elif (AFE_TYPE == bq76xx_afe)
#define UTC_3           ((-28 + 40) * 10)
#endif
#else
#define AFE_UTC           ((-1 + 40) * 10)
#endif // DEBUG
...
/******************************* AFE寄存器结构体 *****************************/
typedef struct {
	UINT8 CN 		:4;  		//5-15，�?�应串数，别的为16�?
	UINT8 BAL 		:1;			//0：平衡功能由内部SH367309控制�?1:由�?�部MCU�?
	UINT8 OCPM 		:1;			//0:充电过流�?关闭充电MOS，放电过流只关放电MOS�?1则同时关
	UINT8 ENMOS 	:1;			/*0:禁�?�充电MOS恢�?�控制位�?1:�?动充电MOS恢�?�控制位�?
												(当过充电/温度保护(温度实际�?2�?)关闭充电MOS后，如果检测到放电状态，则开�?充电MOS) */									
	UINT8 ENPCH 	:1;			//0:禁�?��?�充电功能，1：启动�?�充功能
	
	UINT8 EUVR 		:1;			//0：过放保护状态释放与负载释放无关，意味着负载释放�?和电流保护有关了
	UINT8 OCRA 		:1;			/*0：不允�?��?1：允�?---“电流保护定时恢复”功能，
...
typedef struct {
	UINT8 OVH 		:2;  		//过充保护�?2�?
	UINT8 LDRT 		:2;			//11，负载释放延�?2000ms，这�?和短�?保护释放延时有关�?
	UINT8 OVT		:4;			//过充电保护延�?
	UINT8 OVL;					//过充保护�?8�?
...
typedef struct {
	UINT8 L0V;				//低电压�?��?�充电电�?
	UINT8 PFV;				//二�?�过充电保护电压设置寄存�?，放大一些，不用这个
