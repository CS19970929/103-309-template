...
BMS_PROTECT_PARAM_ADDR = 0x2100
BMS_PROTECT_PARAM_WORDS = 65
BMS_OTHER_PARAM_ADDR = 0x2300
BMS_OTHER_PARAM_WORDS = 32
...
class UpgradeUi(tk.Tk):
    ...
    def _build_params_tab(self, tab: ttk.Frame) -> None:
        ...
        actions = ttk.Frame(tab)
        actions.grid(row=0, column=0, sticky="ew", pady=(0, 6))
        ttk.Label(actions, text="当前项目").grid(row=0, column=0, padx=(0, 16))
        ...
        self._build_param_groups(
            protect_inner,
            [p for p in BMS_PARAM_DEFS if BMS_PROTECT_PARAM_ADDR <= p.addr < BMS_PROTECT_PARAM_ADDR + BMS_PROTECT_PARAM_WORDS],
            columns=3,
        )
        self._build_param_groups(
            other_inner,
            [p for p in BMS_PARAM_DEFS if BMS_OTHER_PARAM_ADDR <= p.addr < BMS_OTHER_PARAM_ADDR + BMS_OTHER_PARAM_WORDS],
            columns=3,
        )
    ...
    def _worker_read_param_range(self) -> None:
        ...
        if self.active_param_range == "protect":
            ranges = [(BMS_PROTECT_PARAM_ADDR, BMS_PROTECT_PARAM_WORDS)]
        elif self.active_param_range == "other":
            ranges = [(BMS_OTHER_PARAM_ADDR, BMS_OTHER_PARAM_WORDS)]
        else:
            ranges = [
                (BMS_PROTECT_PARAM_ADDR, BMS_PROTECT_PARAM_WORDS),
                (BMS_OTHER_PARAM_ADDR, BMS_OTHER_PARAM_WORDS),
            ]
        values: dict[str, int] = {}
        with self._open_client() as client:
            self._set_can_target(client)
